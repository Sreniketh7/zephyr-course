...
static int set_blink_count_impl(const struct device *dev, uint32_t count)
{
    ...
    if (count > OUR_DRIVER_BLINK_COUNT_MAX) {
    ...
    LOG_INF("blink_count set to %u", data->blink_count);
    return 0;
}

/* l6-task1: sample_fetch -> blinks LED blink_count times */
static int our_driver_sample_fetch(const struct device *dev,
                                    enum sensor_channel chan)
{
    struct our_driver_data *data = (struct our_driver_data *)dev->data;

    if (data->blink_count == 0U) {
        LOG_WRN("blink_count is 0 — use 'sensor set <value>' first");
        return 0;
    }

    LOG_INF("sample_fetch: blinking LED %u times", data->blink_count);

    for (uint32_t i = 0U; i < data->blink_count; i++) {
                                   ...
                                   struct sensor_value *val)
{
    ...
    gpio_pin_set_dt(&data->led, 0);
    val->val1 = (int32_t)data->blink_count;  /* return blink_count as the value */
    val->val2 = 0;
    return 0;
}
